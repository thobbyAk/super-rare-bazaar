...
export function createOffer(originContract:Address, tokenId: BigInt, bidder: Account , amount:BigInt, convertible: boolean):OfferLog{
    let id = originContract.toHex().concat('-').concat(tokenId.toString())
    ...
    return offer as OfferLog
} 

export function getOffer(originContract:Address, tokenId: BigInt):OfferLog{
    let id = originContract.toHex().concat('-').concat(tokenId.toString())
    let offer = OfferLog.load(id)
